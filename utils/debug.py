...
def visualizeTraj(agent, local_transition, column_num = 4, delete_num=0, vis_num=9999):
    SMALL_SIZE = 8

    N = vis_num if vis_num < len(local_transition) else len(local_transition)
    fig, axes = plt.subplots(np.ceil(N/column_num).astype(int),column_num)
    ...
    if delete_num > 0:
        for i in range(delete_num):
            fig.delaxes(axes[N//column_num, column_num-1-i])
    plt.show()
