...
class CURLSDQfDCom(CURLDQNCom):
    def __init__(self, lr=1e-4, gamma=0.95, device='cuda', dx=0.005, dy=0.005, dz=0.005, dr=np.pi/32, n_p=1, n_theta=1,
                 z_dim=50, crop_size=64, l=0.1, w=0.1):
        super().__init__(lr, gamma, device, dx, dy, dz, dr, n_p, n_theta, z_dim, crop_size)
        self.margin_l = l
    ...
    def update(self, batch):
        ...
        return (loss.item(), curl_loss.item()), td_error
