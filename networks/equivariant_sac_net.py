...
class EquivariantEncoder128SO2_3(torch.nn.Module):
    def __init__(self, obs_channel=2, n_out=128, initialize=True):
        ...
        self.conv = torch.nn.Sequential(
            # 128x128
            nn.R2Conv(nn.FieldType(self.c4_act, obs_channel * [self.c4_act.trivial_repr]),
                      nn.FieldType(self.c4_act, n_out//8*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//8 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.c4_act, n_out//8*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//8 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.c4_act, n_out//8 * self.repr), 2),
            # 64x64
            nn.R2Conv(nn.FieldType(self.c4_act, n_out//8 * self.repr),
                      nn.FieldType(self.c4_act, n_out//4*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//4 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.c4_act, n_out//4*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//4 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.c4_act, n_out // 4 * self.repr), 2),
            # 32x32
            nn.R2Conv(nn.FieldType(self.c4_act, n_out//4 * self.repr),
                      nn.FieldType(self.c4_act, n_out//2*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//2 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.c4_act, n_out//2*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out//2 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.c4_act, n_out // 2 * self.repr), 2),
            # 16x16
            nn.R2Conv(nn.FieldType(self.c4_act, n_out//2 * self.repr),
                      nn.FieldType(self.c4_act, n_out*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.c4_act, n_out*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.c4_act, n_out * self.repr), 2),
            # 8x8
            nn.R2Conv(nn.FieldType(self.c4_act, n_out * self.repr),
                      nn.FieldType(self.c4_act, n_out*2*4 * [self.c4_act.trivial_repr]) + nn.FieldType(self.c4_act, n_out*2 * self.repr),
    ...
    def __init__(self, obs_channel=2, n_out=128, initialize=True):
        ...
        self.conv = torch.nn.Sequential(
            ...
            nn.R2Conv(nn.FieldType(self.o2, obs_channel * [self.o2.trivial_repr]),
                      nn.FieldType(self.o2, n_out // 8 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 8 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.o2, n_out // 8 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 8 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.o2, n_out // 8 * self.repr), 2),
            # 64x64
            nn.R2Conv(nn.FieldType(self.o2, n_out // 8 * self.repr),
                      nn.FieldType(self.o2, n_out // 4 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 4 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.o2, n_out // 4 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 4 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.o2, n_out // 4 * self.repr), 2),
            # 32x32
            nn.R2Conv(nn.FieldType(self.o2, n_out // 4 * self.repr),
                      nn.FieldType(self.o2, n_out // 2 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 2 * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.o2, n_out // 2 * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out // 2 * self.repr)),
            nn.NormMaxPool(nn.FieldType(self.o2, n_out // 2 * self.repr), 2),
            # 16x16
            nn.R2Conv(nn.FieldType(self.o2, n_out // 2 * self.repr),
                      nn.FieldType(self.o2, n_out * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out * self.repr),
                      kernel_size=3, padding=1, stride=1, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.o2, n_out * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_out * self.repr)),
    ...
    def __init__(self, obs_shape=(2, 128, 128), action_dim=5, n_hidden=128, initialize=True, kernel_size=3):
        ...
        self.critic_1 = torch.nn.Sequential(
            nn.R2Conv(nn.FieldType(self.o2, n_hidden * self.repr + (action_dim - 2) * [self.o2.trivial_repr] + self.n_rho1 * [self.o2.irrep(1, 1)]),
                      nn.FieldType(self.o2, n_hidden * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_hidden * self.repr),
                      kernel_size=1, padding=0, initialize=initialize),
            nn.GatedNonLinearity1(nn.FieldType(self.o2, n_hidden * 4 * [self.o2.trivial_repr]) + nn.FieldType(self.o2, n_hidden * self.repr)),
            nn.NormPool(nn.FieldType(self.o2, n_hidden * self.repr)),
            nn.R2Conv(nn.FieldType(self.o2, n_hidden * 4 * [self.o2.trivial_repr]),
...
if __name__ == '__main__':
    ...
    critic = EquivariantSACCriticO2(obs_shape=(2, 128, 128), action_dim=5, n_hidden=32, initialize=False)
    ...
    actor = EquivariantSACActorSO2_3(obs_shape=(2, 128, 128), action_dim=5, n_hidden=32, initialize=False)
    out2 = actor(o)
    print(1)
